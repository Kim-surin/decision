...
public interface ConversionStrategyDao {
	...
	public List<ConversionStrategyRuleContextDto> selectConversionStrategyRuleContext(Map param);

	// 세번변경기준 충족을 위해 원산지확인서 수취가 필요한 대상(제품과 동일한 호이면서 역외인 원재료)
	public List<Map<String, Object>> selectCthCertifyTargetList(Map param);

	// 부가가치기준 충족 후보(역외 원재료를 재료비 비중 높은 순 정렬) - 실제 필요 건수는 Service에서 계산
	public List<Map<String, Object>> selectValueContentCertifyCandidateList(Map param);
}
