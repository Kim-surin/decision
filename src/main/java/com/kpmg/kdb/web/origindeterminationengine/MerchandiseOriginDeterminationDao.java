...
// "상품"(PRODUCT_ASSETS_TYPE IN ('M','R','B')) 원산지판정 전용 조회/저장. "제품"(P,H)은 ProductOriginDeterminationDao의
// 커서로 기준을 하나씩 적용하지만, 상품은 구매처 원산지확인서/FTA_RULE 조회 결과를 FCR_MST에 반영 후 FCR_RESULT를 기록하는 단순 집합 연산으로 판정한다.
public interface MerchandiseOriginDeterminationDao {
	...
	void insertFcrResultForMerchandise(@Param("salesNo") String salesNo, @Param("divisionCode") String divisionCode,
