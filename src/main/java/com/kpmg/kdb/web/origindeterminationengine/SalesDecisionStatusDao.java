...
public interface SalesDecisionStatusDao {
	 ...
	 * 전체 제품(월 판정) 대상.
	 *
	 * <p>DIVISION_CODE 도 조건에 포함한다 — 가상 SALES_NO는 CUSTOMER_CODE+DIVISION_CODE+YYYYMM을
	 * 구분자 없이 이어붙여 채번하므로(CustomerDivisionYyyymmSalesNoGenerator), COMPANY_CODE+SALES_NO
	 * 만으로는 다른 DIVISION_CODE의 매출까지 함께 갱신될 수 있다.
	 */
