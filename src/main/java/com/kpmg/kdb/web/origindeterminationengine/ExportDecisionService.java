...
/**
 * 수출 개별판정 배치 처리 진입점. 이미 존재하는 실제 SALES_NO 를 그대로 판정 대상으로 삼아 가상매출
 * 생성이 필요 없으므로 데이터 가공(그룹핑) 단계가 없다({@link ExportDecisionTarget} — 호출측이 이미
 * 확정된 대상 목록을 넘긴다). 대상마다 {@link OriginDecisionPipeline} 을 새로 만들어 CREATE_FCR-&gt;
