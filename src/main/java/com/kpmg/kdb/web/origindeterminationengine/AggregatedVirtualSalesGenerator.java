...
/**
 * {@link VirtualSalesGenerator} 기본 구현(내수 전용). 내수(고객사/사업부 그룹) 단위로 가상
 * SALES_MST/SALES_DTL을 만들고 판정대상 커서를 조회하는 {@link AggregatedVirtualSalesDao} 쿼리를
 * 순서대로 호출한다.
 *
 ...
 * 커서가 둘을 함께 조회했기 때문). 이 구현체는 내수 전용이므로, 호출측이 exportFlag 를 세팅했는지와
