...
// 공용으로 쓰이며, SalesOriginDeterminationStatusUpdater가 사용한다.
public interface SalesOriginDeterminationStatusDao {

	// 판정완료 SALES_MST 상태값 갱신. 가상매출(VIRTUAL_YN='Y')이면 무조건 '4', 실제매출(수출 판정대상)이면
	// FCR_MST/FCR_RESULT 오류 존재 여부에 따라 '4' 또는 '5'. productCodes가 null/빈 리스트면 salesNo 전체(월 판정) 대상.
	...
	// 판정 처리 중 예외가 발생한 대상을 판정실패('5')로 표시한다. updateSalesMstDecisionComplete처럼 FCR_MST/FCR_RESULT
	// 내용을 보고 가리지 않고 무조건 '5'다 — 예외가 난 시점엔 그 데이터가 없거나 불완전해 신뢰할 수 없기 때문이다.
