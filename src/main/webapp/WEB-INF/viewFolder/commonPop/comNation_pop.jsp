...
<body>
	<form:form id="comNationPop-form" class="s4-form h-full" novalidate="novalidate" onsubmit="return false;">
		<input type="hidden" id="comNationPop_callback" value="<c:out value='${params.callback}' />">
		<input type="hidden" id="comNationPop_rowIndex" value="<c:out value='${params.rowIndex}' />">
...
<script>
	var COM_NATION_POPUP = new function() {
		...
		this.Initialize_viewObject = function() {
			COM_NATION_POPUP.state['params'] = JSON.parse('${params["data"]}' || '{}');
			...
			COM_NATION_POPUP.retrieve_GridData();
		}
		...
		this.selectData = function (selectedData) {
			var callback = $('#comNationPop_callback').val();
			var rowIndex = $('#comNationPop_rowIndex').val();
	...
	function onPopupClose() {
	    COM_NATION_POPUP.selectData(null);		
	}
