...
<body>
	<form:form id="comItemPop-form" class="s4-form h-full" novalidate="novalidate" onsubmit="return false;">
		<input type="hidden" id="comItemPop_callback" value="<c:out value='${params.callback}' />">
		<input type="hidden" id="comItemPop_rowIndex" value="<c:out value='${params.rowIndex}' />">
...
<script>
	var COM_ITEM_POPUP = new function() {
		...
		this.Initialize_viewObject = function() {
			COM_ITEM_POPUP.state['params'] = JSON.parse('${params["data"]}' || '{}');
			...
			COM_ITEM_POPUP.retrieve_GridData();
		...
		this.createAUIGrid = function() {
			...
			KpackageOBJ.auiGrid.bind(COM_ITEM_POPUP.gridId, "cellDoubleClick", function (event) {
		...
		this.selectData = function (selectedData) {
			var callback = $('#comItemPop_callback').val();
			var rowIndex = $('#comItemPop_rowIndex').val();
	...
	function onPopupClose() {
		COM_ITEM_POPUP.selectData(null);		
	}
