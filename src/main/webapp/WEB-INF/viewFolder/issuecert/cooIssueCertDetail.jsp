	    ...
	    var COO_ISSUE_CERT_DETAIL_POPUP = new function() {
	        ...
	        this.createAUIGrid = function() {
	            const columnLayout = [
	                ...
	                {   dataField: "export_flag", headerText: "export_flag", width: 110, style: "aui-center", headerStyle: "aui-center", visible: false},
	        ...
	        /* 우측 다운로드 버튼 영역 Render */
	        this.renderDownloadArea = function(params) {

	            if (params.export_flag == "D") {
	                params["fta_code"] = "KOREA";
	            }
	            ...
	            KpackageOBJ.ajax.doSubmit("/ireport/retrieveCooIssueDocumentInfo", params, function(result) {
	                ...
	                function makeDocButton(label, iconClass, clickFn, extraParam1, extraParam2, extraParam3) {
	                    var onclickScript = clickFn + "(";

	                    var args = [];
	                    if (typeof extraParam1 !== "undefined") args.push("'" + String(extraParam1).replace(/'/g, "\\'") + "'");
	                    if (typeof extraParam2 !== "undefined") args.push("'" + String(extraParam2).replace(/'/g, "\\'") + "'");
	                    if (typeof extraParam3 !== "undefined") args.push("'" + String(extraParam3).replace(/'/g, "\\'") + "'");

	                    onclickScript += args.join(",") + ")";

	                    return ''
	                        + '<button type="button" '
	                        + 'class="btn p-0 border border-transparent bg-transparent d-flex flex-column align-items-center justify-content-start download-icon-btn" '
	                        + 'onclick="' + onclickScript + '">'
	                        + '    <svg class="sa-icon ' + iconClass + '" style="width: 30px; height: 30px;">'
	                        + '        <use href="/rcs/ui5x/img/sprite.svg#file"></use>'
	                        + '    </svg>'
	                        + '    <span class="mt-1 small fw-semibold text-dark text-center download-icon-label" '
	                        + '          style="font-size: 12px; line-height: 1.2; min-height: 30px; white-space: normal; word-break: break-word;">'
	                        +              label
	                        + '    </span>'
	                        + '</button>';
	                }

	                for (var inx = 0; inx < ftaDocumentList.length; inx++) {
	                    var row = ftaDocumentList[inx];
	                    var formName = row.form_name || "문서";
	                    var formId = row.form_id || "";
	                    var formFileName = row.form_file_name || "";
