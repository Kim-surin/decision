	...
	<div class="content-wrapper">
		<div class="row">
			<div class="content-wrapper col-3">
				<h1 class="subheader-title mb-1">수동 실행</h1>
					...
					<div class="panel-container show">
						<div class="panel-content">
							<div class="row">
								...
								<div class="col"></div>
								<div class="col-1">
									<button type="button" onclick="javascript:IFMANUAL000.executeInterface();" class="btn btn-sm btn-primary waves-effect waves-themed mt-2">실행요청</button>
								</div>
							</div>
						</div>
					</div>
				...
				<div id="panel-5" class="panel panel-icon">
					<div class="panel-container show">
						<div class="panel-content">
							<div class="row">
								<div class="col-3">
									<label class="form-label">수행일자</label>
									<div class="row">
								...
								<div class="col-lg-5">
									...
									<div class="row mb-3">
										...
										<div class="col"><input type="text" id="search_keyword" name="search_keyword" class="form-control" placeholder="키워드 입력" onkeydown="if(event.keyCode===13){IFMANUAL000.retrieveResultList();}"></div>
									</div>
								</div>
								<div class="col"></div>
								<div class="col-1">
									<button type="button" onclick="javascript:IFMANUAL000.retrieveResultList();" class="btn btn-sm btn-search search-no-more waves-effect waves-themed mt-2">Search</button>
								</div>
							</div>
						</div>
					</div>
		...
		<div class="row">
			<div class="col-12">
				<h6 class="mb-2">인터페이스 수행결과</h6>
				<div id="oAuiGrid_IFMANUAL000_01" style="width:100%;height:280px; margin:0 auto;"></div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-12">
				<div id="oAuiGrid_IFMANUAL000_02" style="width:100%;height:420px; margin:0 auto;"></div>
			</div>
		</div>
	</div>
</body>
	...
	var IFMANUAL000 = new function() {
		...
		this.createResultGrid = function() {
			const columnLayout = [
				...
				{ dataField:"if_name", headerText:"인터페이스 명", width:170, filter:{showIcon:true} },
				{ dataField:"total_rows", headerText:"이관 건수", width:100, dataType:"numeric", style:"aui-right" },
				{ dataField:"trans_status_name", headerText:"상태", width:100 },
				{ dataField:"start_time", headerText:"이관 시작", width:180 },
				{ dataField:"trans_date", headerText:"이관 종료", width:180 },
				{ dataField:"if_param", headerText:"요청파라메터", width:400 },
				{ dataField:"error_message", headerText:"오류내용", width:300 }
			];
			const gridProps = { selectionMode:"singleRow", usePaging:true, pageRowCount:50, showPageRowSelect:true, enableFilter:true };
		...
		this.executeInterface = function() {
			...
			KpackageOBJ.ajax.doSubmit("/system/interface/manualExecution/executeInterface", params, function(result) {
				...
				if (result.success) IFMANUAL000.retrieveResultList();
			});
		}

		this.retrieveResultList = function() {
			$("#intg_interface_trans_id, #detail_if_code").val("");
			if (IFMANUAL000.grid_IFMANUAL000_02) AUIGrid.setGridData(IFMANUAL000.grid_IFMANUAL000_02, []);
			var params = KpackageOBJ.data.makePostData("IFMANUAL000-form-02");
			KpackageOBJ.auiGrid.retrieve(IFMANUAL000.grid_IFMANUAL000_01, "/system/interface/manualExecution/retrieveExecutionResultList", params);
		}

		this.retrieveExecutionDetail = function() {
			if (!$("#intg_interface_trans_id").val()) {
				alert("조회할 수행결과를 선택해 주세요.");
				return;
			}
