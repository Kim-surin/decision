...
<div id="content">
	<div class="widget-body">
		<form:form id="MM00301-detail-form" class="s4-form form-horizontal form-dialog" novalidate="novalidate">
						...
						<table class="table table-bordered">
							...
							<tbody>
								<tr>
									<th>협력사코드</th>
									<td colspan="3">
										<input type="text" id ="VENDOR_CODE" name="VENDOR_CODE" class="form-control" placeholder="ex) ABCD"/>
									</td>
								</tr>
								<tr>
									<th>협력사명</th>
...
<script>
	...
	var MM00301 = new function(){
		
		this.DIALOG_ID = "vendor_Dtl";
		// Page Object Initialize
		this.initialize_Dialog_Object = function() {
			if('${reqParam.INPUT_FLAG}' == "U"){
				KpackageOBJ.object.readOnly("MM00301-detail-form", "VENDOR_CODE", true);
			}
			
			MM00301.retrieve_Dialog_formData();
		};
		
		
		this.retrieve_Dialog_formData = function(){
			var params = { "VENDOR_CODE" : "${reqParam.VENDOR_CODE}"}; 
			KpackageOBJ.ajax.doSubmit("/master/retrieveVendorDetail", params, MM00301.formData_Handler);	
		}
		
		this.formData_Handler = function(result) {
			if (result.success) { // 성공시			
				var data = result.value;
				KpackageOBJ.data.setFormData("MM00301-detail-form", data);
			} else { // 실패시
		...
		this.dataSaveFunction = function(){
			var postData = KpackageOBJ.data.makePostData("MM00301-detail-form");
			KpackageOBJ.ajax.doSubmit("/master/updateVendorDetail", postData, "MM00301.dataSaveFunction_CallbackHandler");
		}
		
		this.dataSaveFunction_CallbackHandler = function(result) {
			if (result.success) { // 성공시
				MM003.retrieve_gridData();
				alert("저장되었습니다.");
	            KpackageOBJ.dialog.close(MM00301.DIALOG_ID);
			} else { // 실패시
	...
	$(document).ready(function() {
		MM00301.initialize_Dialog_Object();		// 화면에서 사용하는 Selelect Box, Calendar 등을 생성합니다. 
		
		var tools = [{icon:"save", title:"Save" ,text:"저장"	,func:"MM00301.dataSaveFunction"}];
		KpackageOBJ.dialog.setButton(MM00301.DIALOG_ID, tools);
	});
