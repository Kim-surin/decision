...
var KpackageOBJ = {
  ...
  grid: {
    ...
    create: function (
      ...
      if (
        addOptObject["rownum"] != null &&
        addOptObject["rownum"] != undefined
      ) {
      ...
      var gridOptObject = {
        ...
        loadComplete: function (data) {
          ...
          if ($.browser.msie) {
            var patchWidth = $(
              "[aria-labelledby='gbox_" + $(this).prop("id") + "']"
            ).css("width");
            var patchTarget = $(this).parent();
            $(patchTarget).css("width", patchWidth);
          }
          if (
            p_loadComplete_Callback != null &&
            p_loadComplete_Callback != undefined &&
            p_loadComplete_Callback != ""
          ) {
        ...
        onSelectRow: function (rowid, status, e) {
          if (
            p_click_function != null &&
            p_click_function != undefined &&
            p_click_function != ""
          ) {
        ...
        ondblClickRow: function (rowid) {
          //grid double click evnet
          if (
            p_dblClick_function != null &&
            p_dblClick_function != undefined &&
            p_dblClick_function != ""
          ) {
    ...
    getCheckedRows: function () {
      ...
      for (
        var i = 0;
        i < selRow.length;
        i++ //iterate through array of selected rows
      ) {
    ...
    changDatagridToJson: function (gridData) {
      ...
      if (
        gridData != null &&
        gridData != "undefined" &&
        (gridData.length == null || gridData.length == "undefined")
      ) {
    ...
    setButton: function (gridId, buttonObject) {
      ...
      if (oUtil.isArray(buttonObject)) {
        ...
        for (var i = 0; i < buttonObject.length; i++) {
          ...
          iconValue +=
            "<a class='btn grid-add-btn btn-default btn-border' id='" +
            gridId +
            text +
            func +
            "' href='javascript:" +
    ...
    getGridColumns: function (gridId) {
      ...
      for (var i = 0; i < colModel.length; i++) {
        ...
        var rowspan = null;
        var colspan = null;
        if (field == "rn" || field == "cb") {
          //rownum하고 체크박스는 해더로 만들지 않는다.
          continue;
        }
        ...
        //toMap.push("{");
        if (!oUtil.isNull(field)) toMap.push('"field":"' + field + '",');
        if (!oUtil.isNull(title))
          toMap.push('"title":"' + title.toString().replace("<br>", "") + '",');
        if (!oUtil.isNull(width))
          toMap.push('"width":"' + parseInt(width * 32) + '",');
        if (!oUtil.isNull(align)) toMap.push('"align":"' + align + '",');
        if (!oUtil.isNull(halign)) toMap.push('"halign":"' + halign + '",');
        ...
        if (!oUtil.isNull(editor)) toMap.push('"editor":"' + editor + '",');
        if (!oUtil.isNull(checkbox))
          toMap.push('"checkbox":"' + checkbox + '",');
        if (!oUtil.isNull(formatter)) toMap.push('"formatter":"",');
        ...
        toMap[toMap.length - 1] = lastMapVal.substring(
          0,
          lastMapVal.length - 1
        );
    ...
    toggleGrid: function (gridId) {
      ...
      $(
        "#gview_" +
          gridId +
          " > div.ui-jqgrid-titlebar.ui-corner-top.ui-helper-clearfix > a > span"
      ).trigger("click");
    },
